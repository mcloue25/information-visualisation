...
def make_latest_snapshot(df):
    valid = df.dropna(subset=["renewables_share_energy", "fossil_share_energy", "population"]).copy()
    ...
    print(f"Using latest snapshot year: {latest_year}")
    return latest
...
def make_story_countries(latest):
    leaders = latest.sort_values("renewables_share_energy", ascending=False).head(3)["country"].tolist()
    laggards = latest.sort_values("renewables_share_energy", ascending=True).head(3)["country"].tolist()
    mid = ["Ireland", "Germany"] if "Ireland" in latest["country"].values and "Germany" in latest["country"].values else []

    selected = []
    for c in leaders + laggards + mid:
...
def plot_scatter_latest(latest):
    ...
    labels = ["Denmark", "Germany", "Ireland", "United Kingdom", "China", "India", "United States", "Brazil", "Sweden", "Norway", "Iceland"]
    for _, row in plot_df[plot_df["country"].isin(labels)].iterrows():
        ax.annotate(
            row["country"],
    ...
    ax.set_title("Renewables vs Fossil Dependency")
...
def plot_focus_transition_gap(df, countries):
    plot_df = df[df["country"].isin(countries)].dropna(subset=["clean_share_elec", "fossil_share_elec"]).copy()
    ...
    for country in countries:
        ...
        if not sub.empty:
            ax.plot(
                sub["year"],
                sub["clean_share_elec"] - sub["fossil_share_elec"],
                label=country
            )

    ax.axhline(0, linewidth=1)
    ax.set_title("How Has the Transition Evolved? Clean Electricity Share Minus Fossil Share")
    ax.set_xlabel("Year")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOT_DIR, "focus_transition_gap.png"), dpi=300)
    plt.close()



def save_story_outputs(df, latest, improvers):
    story_countries = make_story_countries(latest)
    line_df = df[df["country"].isin(story_countries)].copy()

    df.to_csv(os.path.join(DATA_OUT_DIR, "energy_master.csv"), index=False)
    latest.to_csv(os.path.join(DATA_OUT_DIR, "energy_latest_snapshot.csv"), index=False)
    improvers.to_csv(os.path.join(DATA_OUT_DIR, "energy_top_improvers.csv"), index=False)
    line_df.to_csv(os.path.join(DATA_OUT_DIR, "energy_story_countries.csv"), index=False)

    print("Story countries:", story_countries)
    return story_countries
...
def plot_top_improvers(improvers):
    if improvers.empty:
        print("No data available for top improvers plot.")
        return

    plot_df = (
        improvers.sort_values("renewables_share_energy_change", ascending=False)
    ...
    ax.scatter(plot_df["renewables_share_energy_2000"], y, s=40, label="2000")
    ax.scatter(plot_df["renewables_share_energy_latest"], y, s=40, label="Latest")

    for i, row in plot_df.iterrows():
        ax.text(
            row["renewables_share_energy_latest"] + 0.8,
            i,
            f"+{row['renewables_share_energy_change']:.1f}",
            va="center",
            fontsize=8
        )
    ...
    ax.set_title("Who Improved Most? Largest Increases in Renewable Share Since 2000")
    ax.legend()
...
def plot_latest_lollipop(latest, n=15):
    plot_df = (
        latest.sort_values("renewables_share_energy", ascending=False)
        .head(n)
        .reset_index(drop=True)
    )
    ...
    ax.hlines(y=y, xmin=0, xmax=values, linewidth=2)
    ...
    ax.set_xlabel("Renewables share of primary energy (%)")
    ax.set_title("Who Leads Today? Top Countries by Renewable Energy Share")
    ax.xaxis.grid(True, linestyle="--", alpha=0.4)
...
def main():
    ...
    story_countries = save_story_outputs(df, latest, improvers)

    plot_latest_lollipop(latest, n=15)
    plot_top_improvers(improvers)
    plot_scatter_latest(latest)
    plot_focus_transition_gap(df, story_countries)
